    ...
    public static List<Token> findComparativeIndicator(Sentence curSentence, NounPhrase np1, NounPhrase np2) {
        ...
        while (i < tokens.size()) {
            ...
            if (token.getOffsetBegin() > offsetTraverseBegin && token.getOffsetEnd() < offsetTraverseEnd) {
                ...
                //Case 5: same + N + as
                if (np1 != null && np2 != null && isNoun(token)) {
...
//                    return 2;
//                }
//                if (np2.getSentenceId() - np1.getSentenceId() == 1
//                        && !hasProperName(np1, StanfordUtil.reviews.get(np1.getReviewId()).getSentences().get(np1.getSentenceId()))) {
//                    return 2;
//                }
//                if (np2.getSentimentOrientation() == Util.NEUTRAL
//                        || np1.getSentimentOrientation() == Util.NEUTRAL) {
//                    return 2;
//                } else if (np2.getSentimentOrientation() == np1.getSentimentOrientation()) {
//                    return 0;
//                } else if (np2.getSentimentOrientation() != np1.getSentimentOrientation()) {
//                    return 1;
//                }
...
//    }
    ...
    public static int sentimentConsistencyExtract(NounPhrase np1, NounPhrase np2) {
        if (np1.getReviewId() == np2.getReviewId()) {
            if (Math.abs(np1.getSentenceId() - np2.getSentenceId()) == 1) {
                //Sentiment to NP1 and NP2 must be the same. If not, return 2
                if (np2.getSentimentOrientation() != Util.NEUTRAL
                        && np1.getSentimentOrientation() != Util.NEUTRAL) {
                    //NP1 is in the sentence right after the NP2's one
                    if (np1.getSentenceId() - np2.getSentenceId() == 1) {
                        //NP2 is an object
                        if (np2.getType() == 0) {
                            //NP1 is a candidate
                            if (np1.getType() == 2) {
                                if (np2.getSentimentOrientation() == np1.getSentimentOrientation()) {
                                    return 0;
                                } else if (np2.getSentimentOrientation() != np1.getSentimentOrientation()) {
                                    return 1;
                                }
                    ...
                    //NP2 is in the sentence right after the NP1's one
                    if (np2.getSentenceId() - np1.getSentenceId() == 1) {
                        //NP1 is an object
                        if (np1.getType() == 0) {
                            //NP2 is a candidate
                            if (np2.getType() == 2) {
                                if (np1.getSentimentOrientation() == np2.getSentimentOrientation()) {
                                    return 0;
                                } else if (np1.getSentimentOrientation() != np2.getSentimentOrientation()) {
                                    return 1;
                                }
                            }
                        }
