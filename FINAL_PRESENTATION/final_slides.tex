...
\begin{document}
	...
	\begin{frame}
		...
		\begin{block}{Động cơ thực hiện đề tài}
			\begin{itemize}	
				...
				\item{Cải thiện 10\% kết quả Khai khoáng ý kiến (theo Nicolas Nicolov \footnotemark).}
			\end{itemize}					
		\end{block}		
		\begin{block}{Ví dụ}						
	...
	\begin{frame}
		...
		\begin{block}{Phạm vi đề tài}
			...
			\end{itemize}
			\footnotetext[2]{Minqing Hu and Bing Liu. 2004.
			...
			\footnotetext[3]{A-M. Popescu and O. Etzioni. 2005. 
	...
	\begin{frame}
		...
		\begin{block}{Đồng tham chiếu trong khai khoáng ý kiến}
			...
			\end{itemize}
		\end{block}
		\footnotetext[4]{Veselin Stoyanov and Claire Cardie. 2006.
		...
		\footnotetext[5]{Xiaowen Ding and Bing Liu. 2010.
	...
	\begin{frame}{Tiền xử lý}		
		...
		\begin{columns}[t]
			...
			\end{column}			
		\end{columns}
		\footnotetext[6]{http://stanfordnlp.github.io/CoreNLP}
		\footnotetext[7]{http://web.mit.edu/6.863/www/PennTreebankTags.html}
	\end{frame}
	...
	\begin{frame}{Trích xuất cụm danh từ}		
		\begin{columns}[t]
			...
			\end{column}
		\end{columns}
		\footnotetext[8]{http://crfchunker.sourceforge.net}
		\begin{columns}[t]
			\begin{column}{\textwidth}
	...
	\section{Phụ lục}
		...
		\begin{frame}{Phụ lục: Công cụ CRFChunker}
			...
			\end{figure}
			\footnotetext{Xuan-Hieu Phan, \textit{CRFChunker: CRF English Phrase Chunker}, http://crfchunker.sourceforge.net/, 2006.}
		\end{frame}
		...
		\begin{frame}{Phụ lục: Từ điển ý kiến và từ khóa so sánh}
			...
			\begin{block}{Từ khóa so sánh}
				...
				\end{itemize}
			\end{block}
			\footnotetext{http://www.cs.uic.edu/~liub/FBS/opinion-lexicon-English.rar}
		\end{frame}

		\begin{frame}{Công trình của Nicolov}
		\end{frame}

		\begin{frame}{Công trình của Stoyanov và Cardie}
		\end{frame}

		\begin{frame}{Công trình của Ding và Liu (2010)}
			\begin{figure}[H]
				\centering							
		...
		\begin{frame}{Công trình của Ding và Liu (2010) (tt)}
			\begin{figure}[H]
				\centering							
		...
		\begin{frame}{Các công trình tìm đối tượng và thuộc tính}
		...
		\begin{frame}{Mô hình cặp (mention pair)}
			\begin{figure}[H]
				\centering
		...
		\begin{frame}{Mô hình hướng thực thể (entity-mention)}
			\begin{figure}[H]
				\centering
		...
		\begin{frame}{Mô hình xếp hạng (ranking model)}
			\begin{figure}[H]
				\centering
